...
# Fase 6 — Trazabilidad científica (Snapshots, Citación, Fork, Compare)

**Objetivo:** soportar el ciclo "congelar–citar–reusar" propio de la
investigación reproducible.

## 6.1 Backend

### Dominio (`Releases` context)

- `Snapshot { Id, StudyId, Version (SemVer), Description, CreatedBy,
  CreatedAt, MinIOPath, ManifestHash }`.
- `Citation { SnapshotId, BibTex, Apa, Ris, ShortId }`.
- En `Study`: `ForkedFromId? `, `ForkedFromSnapshotId?`.

### Servicios

- `SnapshotPackager` (background): empaqueta estudio + trazas + anotaciones
  + resultados en un manifiesto inmutable en MinIO.
- `ForkOrchestrator`: clona registros + copia paths en MinIO de forma
  perezosa (copy-on-write si el origen es público).
- `SequenceDiffEngine`: alineamiento + diff por bloques (reutiliza
  `geneflow-analysis`).

### Eventos

- `SnapshotCreatedEvent`, `StudyForkedEvent`, `CitationGeneratedEvent`.

### API

- `POST /api/studies/{id}/snapshots` (body: `{ version, description }`).
- `GET /api/studies/{id}/snapshots`.
- `GET /api/snapshots/{id}` · `GET /api/snapshots/{id}/citation?format=…`.
- `POST /api/studies/{id}/fork`.
- `GET /api/compare?left={traceIdOrSnapshotId}&right=…&mode=sequence|study`.

## 6.2 Frontend

### Tipos y servicios

- `app/types/releases.ts`, `app/types/compare.ts`.
- `app/services/releases.service.ts`, `fork.service.ts`,
  `compare.service.ts`.

### Hooks

- `use-snapshots.ts`, `use-citation.ts`, `use-fork.ts`, `use-compare.ts`.

### Componentes de feature

- `app/components/releases/SnapshotList.tsx`, `SnapshotDetail.tsx`,
  `CreateSnapshotDialog.tsx` (con sugerencia de SemVer).
- `app/components/releases/CitationDialog.tsx` (tabs BibTeX / APA / RIS +
  copy-to-clipboard).
- `app/components/releases/ForkButton.tsx` (con confirmación si el
  estudio es grande).
- `app/components/compare/CompareView.tsx` (dos paneles sincronizados).
- `app/components/compare/SequenceDiff.tsx` (renderizado tipo diff,
  reutiliza el visor existente de secuencias).

### Páginas

- Pestaña *Releases* en la página de estudio.
- Botón *Cite* en cabecera de estudio y de snapshot.
- Botón *Fork* en estudios públicos.
- **Nueva** `(platform)/compare/page.tsx?left=…&right=…`.

### i18n

- `releases.*`, `citation.*`, `fork.*`, `compare.*`.

## 6.3 Tests

- **BE unit**: cálculo del `ManifestHash` determinista, validación
  SemVer, generación de `BibTeX/APA/RIS` desde `Citation`, lógica
  de `SequenceDiffEngine` con casos sintéticos.
- **BE integración**: `SnapshotPackager` escribiendo a MinIO real
  (Testcontainers), `ForkOrchestrator` copiando registros + paths
  (copy-on-write para públicos), permisos heredados tras el fork.
- **BE E2E API**: `POST /api/studies/{id}/snapshots`,
  `GET /api/snapshots/{id}/citation?format=…`, `POST .../fork`,
  `GET /api/compare?left=…&right=…` con permisos y errores.
- **FE E2E**: crear snapshot con sugerencia de versión, copiar
  BibTeX al portapapeles, fork de estudio público y aparición en
  perfil del usuario, vista compare con diff sincronizado.
