...
/// <summary>
/// Unit tests for UndoAllSequenceEditsCommandHandler.
/// </summary>
public class UndoAllSequenceEditsCommandHandlerTests
{
    ...
    private readonly UndoAllSequenceEditsCommandHandler _handler;

    public UndoAllSequenceEditsCommandHandlerTests()
    {
        _unitOfWork.Traces.Returns(_traceRepository);
        _unitOfWork.SaveChangesAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(1));

        _handler = new UndoAllSequenceEditsCommandHandler(_unitOfWork);
    }

    #region Success Cases

    [Fact]
    public async Task Handle_WithActiveEdits_ShouldUndoAllSuccessfully()
    {
        // Arrange
        var trace = CreateProcessedTraceWithMultipleEdits();
        var command = new UndoAllSequenceEditsCommand(
            "U00000001",
            trace.Id.ToString());
    ...
    [Fact]
    public async Task Handle_WithNoActiveEdits_ShouldFail()
    {
        // Arrange
        var trace = CreateProcessedTrace(); // No edits
        var command = new UndoAllSequenceEditsCommand(
            "U00000001",
            trace.Id.ToString());
