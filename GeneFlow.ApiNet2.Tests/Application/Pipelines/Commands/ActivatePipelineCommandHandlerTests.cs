...
/// <summary>
/// Unit tests for ActivatePipelineCommandHandler.
/// </summary>
public class ActivatePipelineCommandHandlerTests
{
    private readonly IPipelineRepository _pipelineRepository;
    private readonly IPipelineUnitOfWork _unitOfWork;
    private readonly ActivatePipelineCommandHandler _handler;
    ...
    private static Pipeline CreateDraftPipelineWithoutSteps(UserId ownerId)
    {
        var pipelineId = new PipelineId(1);
        var studyId = new StudyId(1);
        var name = PipelineName.Create("Test Pipeline").Value;
    ...
    private static Pipeline CreateDraftPipelineWithOnlyDisabledSteps(UserId ownerId)
    {
        var pipelineId = new PipelineId(1);
        var studyId = new StudyId(1);
        var name = PipelineName.Create("Test Pipeline").Value;
        ...
        var pipeline = Pipeline.Create(pipelineId, studyId, ownerId, name, description).Value;
        var config = StepConfiguration.Create("{}", StepType.Quality).Value;
        pipeline.AddStep(StepType.Quality, config, "Disabled Step", false, ownerId);
    ...
    [Fact]
    public async Task Handle_WithDraftPipelineWithSteps_ShouldReturnSuccessWithActivatedPipeline()
    {
        // Arrange
        ...
        var command = new ActivatePipelineCommand(
            ValidUserId,
            ValidStudyId,
            ValidPipelineId);
        ...
        result.Value.StatusId.Should().Be(PipelineStatus.Active.Id);
        result.Value.StatusName.Should().Be(PipelineStatus.Active.DisplayName);
    }
    ...
    [Fact]
    public async Task Handle_WithValidPipeline_ShouldSetCanBeExecutedToTrue()
    {
        // Arrange
        var ownerId = new UserId(1);
        var pipeline = CreateDraftPipelineWithSteps(ownerId);
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.CanBeExecuted.Should().BeTrue();
        result.Value.CanBeEdited.Should().BeFalse();
    }
    ...
    #region Handle - No Steps Configured

    [Fact]
    public async Task Handle_WhenPipelineHasNoSteps_ShouldReturnNoStepsError()
    {
        // Arrange
        var ownerId = new UserId(1);
        var pipeline = CreateDraftPipelineWithoutSteps(ownerId);
    ...
    [Fact]
    public async Task Handle_WhenPipelineHasOnlyDisabledSteps_ShouldReturnNoStepsError()
    {
        // Arrange
        var ownerId = new UserId(1);
        var pipeline = CreateDraftPipelineWithOnlyDisabledSteps(ownerId);
    ...
    #region Handle - Already Active

    [Fact]
    public async Task Handle_WhenPipelineIsAlreadyActive_ShouldReturnInvalidTransitionError()
    {
        // Arrange
        var ownerId = new UserId(1);
    ...
    [Fact]
    public async Task Handle_WhenActivationFails_ShouldNotCallSaveChanges()
    {
        // Arrange
        var ownerId = new UserId(1);
        var pipeline = CreateDraftPipelineWithoutSteps(ownerId);
    ...
    public async Task Handle_WithValidPipeline_ShouldUpdateModifiedTimestamp()
    {
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        // The pipeline's ModifiedAt should be updated after activation
